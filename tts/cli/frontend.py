...
class CosyVoiceFrontEnd:
    ...
    def __init__(self,
        ...
        if self.use_ttsfrd:
            ...
            assert self.frd.initialize('{}/../../pretrained_models/CosyVoice-ttsfrd/resource'.format(ROOT_DIR) if ttsfrd_resource_path == '' else ttsfrd_resource_path) is True, \
    ...
    def _extract_speech_token(self, speech):
        ...
        speech_token = self.speech_tokenizer_session.run(None,  # 使用 ONNX 模型将语音特征转换为 speech token 序列，词表大小为4096；就是论文中的speech tokenizer
    ...
    def frontend_sft(self, tts_text, spk_id):
        ...
        if spk_id is None or spk_id not in self.spk2info or self.spk2info == {}:
    ...
    def frontend_zero_shot(self, tts_text, prompt_text, prompt_speech_16k, resample_rate):
        ...
        speech_feat, speech_feat_len = self._extract_speech_feat(prompt_speech_resample)  # 提取prompt_speech_resample的mel谱图特征
        speech_token, speech_token_len = self._extract_speech_token(prompt_speech_16k)  # 提取prompt_speech_16k的speech token序列
